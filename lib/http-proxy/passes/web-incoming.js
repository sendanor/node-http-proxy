...
[ // <--
  ...
  function timeout(req, res, options) {
	var timeout = (options && options.options && options.options.timeout) || options.timeout;
    if(timeout) {
  ...
  function XHeaders(req, res, options) {
    if(!(options.xfwd || (options.options && options.options.xfwd))) return;
    ...
    ['for', 'port', 'proto'].forEach(function(header) {
	  if(req.headers['x-forwarded-' + header]) {
		options.headers['x-forwarded-' + header] = req.headers['x-forwarded-' + header];
	  }
      options.headers['x-forwarded-' + header] =
        (options.headers['x-forwarded-' + header] || '') +
        (options.headers['x-forwarded-' + header] ? ',' : '') +
        values[header];
    });
